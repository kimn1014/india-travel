  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-16">
      ...
      <section className="mb-10 sm:mb-24">
        {/* Title + Countdown combined for mobile */}
        <div className="text-center mb-8 sm:mb-14">
          <p className="text-[10px] sm:text-sm text-neutral-500 tracking-wide mb-3 sm:mb-6">
          ...
          {mounted && (
            <div className="mt-4 sm:mt-8">
              ...
              <p className="text-xs sm:text-base text-neutral-400 mt-2 sm:mt-3 tabular-nums">
        ...
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-4 sm:p-8">
          <h3 className="text-[10px] sm:text-xs font-medium text-neutral-400 uppercase tracking-wider mb-3 sm:mb-5">여행 루트</h3>
          <div className="flex flex-wrap items-center justify-center gap-1.5 sm:gap-3">
            {cities.map((city, idx) => (
              <div key={idx} className="flex items-center gap-1.5 sm:gap-3">
                <span className="text-xs sm:text-base font-medium">{city}</span>
                {idx < cities.length - 1 && (
                  <span className="text-neutral-300 dark:text-neutral-600 text-xs sm:text-base">&rarr;</span>
                )}
              </div>
            ))}
          </div>
          <div className="grid grid-cols-4 gap-2 sm:gap-4 mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-neutral-100 dark:border-neutral-800">
            <div className="text-center">
              <p className="text-xl sm:text-3xl font-bold">{tripInfo.totalDays}</p>
              <p className="text-[10px] sm:text-xs text-neutral-500 mt-0.5 sm:mt-1">일정</p>
            </div>
            <div className="text-center">
              <p className="text-xl sm:text-3xl font-bold">5</p>
              <p className="text-[10px] sm:text-xs text-neutral-500 mt-0.5 sm:mt-1">도시</p>
            </div>
            <div className="text-center">
              <p className="text-xl sm:text-3xl font-bold">6</p>
              <p className="text-[10px] sm:text-xs text-neutral-500 mt-0.5 sm:mt-1">항공편</p>
            </div>
            <div className="text-center">
              <p className="text-xl sm:text-3xl font-bold">-3.5h</p>
              <p className="text-[10px] sm:text-xs text-neutral-500 mt-0.5 sm:mt-1">시차</p>
            </div>
          </div>
        </div>
      </section>
          ...
          {[
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-semibold text-sm sm:text-lg">{item.label}</h3>
          ...
          {itinerary.slice(0, 6).map((day) => (
            ...
            >
              <div className="flex items-start gap-2.5 sm:gap-3">
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-neutral-900 dark:bg-white flex items-center justify-center text-white dark:text-neutral-900 font-bold text-xs sm:text-sm shrink-0">
                  D{day.day}
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  <h3 className="font-semibold text-xs sm:text-base truncate">{day.title}</h3>
